...
//	}
	...
	@ResponseBody
	@PostMapping("/findOneDayTime/{year}/{month}/{day}")   //統計日銷售額圖表
	public List<List<String>> findOneDayTime(@PathVariable(value="year") String year, @PathVariable(value="month") String month, @PathVariable(value="day") String day){ //統計數量圖表
	    ...
	    Integer [] proId = detailService.findProductIdToDoNextStep(n1, n2);
	    
	    int proIdSize = proId.length;//3
	    int p1 = proId[0];
	    int plast = proId[proIdSize-1];
	    
	    List<List<String>> a = detailService.findqqq(p1,plast);
	    return a;
