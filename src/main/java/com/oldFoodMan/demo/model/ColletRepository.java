...
public interface ColletRepository extends JpaRepository<FoodRecord, Integer> {
																//=?
	@Query(value="select * from FoodRecord where shopType like %:shopType% or title like %:shopType% or content like %:shopType% or shopName like %:shopType% or city like %:shopType%",nativeQuery = true)
